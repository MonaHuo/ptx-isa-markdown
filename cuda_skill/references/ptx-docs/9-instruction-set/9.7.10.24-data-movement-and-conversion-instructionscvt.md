---
title: "9.7.10.24. Data Movement and Conversion Instructions: cvt"
section: "9.7.10.24"
version: "9.4"
url: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-cvt
---

<a id="data-movement-and-conversion-instructions-cvt"></a>

<a id="id282"></a>

####  9.7.10.24. Data Movement and Conversion Instructions: `cvt`
...
cvt.frnd2{.relu}{.satfinite}{.pzo}.f16x2.f32     d, a, b;
cvt.rs{.relu}{.satfinite}.f16x2.f32              d, a, b, rbits;

cvt.frnd2{.relu}{.satfinite}{.pzo}.bf16.f32      d, a;
cvt.frnd2{.relu}{.satfinite}{.pzo}.bf16x2.f32    d, a, b;
cvt.rs{.relu}{.satfinite}.bf16x2.f32             d, a, b, rbits;

cvt.rna{.satfinite}.tf32.f32                     d, a;
cvt.frnd2{.satfinite}{.relu}{.pzo}.tf32.f32      d, a;

cvt.frnd2.satfinite{.relu}{.pzo}{.scaled::n1::ue8m0}.f8x2type.f32       d, a, b{, scale-factor};
cvt.frnd2.satfinite{.relu}{.pzo}{.scaled::n1::ue8m0}.f8x2type.fp16x2    d, a{, scale-factor};
...
cvt.frnd2.satfinite{.relu}{.pzo}{.scaled::n1::ue8m0}.f4x2type.f32        d, a, b{, scale-factor};
cvt.frnd2.satfinite{.relu}{.pzo}{.scaled::n1::ue8m0}.f4x2type.fp16x2type d, a{, scale-factor};
...
cvt.frnd2.satfinite{.relu}{.pzo}{.scaled::n1::ue8m0}.f6x2type.f32        d, a, b{, scale-factor};
cvt.frnd2.satfinite{.relu}{.pzo}{.scaled::n1::ue8m0}.f6x2type.fp16x2type d, a{, scale-factor};
...
cvt.frnd2.satfinite{.relu}{.scaled::n2::ue8m0}.s2f6x2.f32     d, a, b{, scale-factor};
cvt.frnd2.satfinite{.relu}{.scaled::n2::ue8m0}.s2f6x2.bf16x2  d, a{, scale-factor};
cvt.rn{.satfinite}{.relu}{.scaled::n2::ue8m0}.bf16x2.s2f6x2   d, a{, scale-factor};

cvt.frnd4{.satfinite}.ue5m3x2.f32                             d, a, b;
cvt.frnd2{.satfinite}.scaled::n1::ue8m0.ue5m3x2.f32           d, a, b, scale-factor;

cvt.frnd4{.satfinite}.ue5m3x2.fp16x2type                      d, a;
cvt.frnd2{.satfinite}.scaled::n1::ue8m0.ue5m3x2.fp16x2type    d, a, scale-factor;

cvt.rn.f16x2.ue5m3x2                                          d, a;
cvt.rn{.satfinite}{.scaled::n2::ue8m0}.bf16x2.ue5m3x2         d, a{, scale-factor};
...
Optional qualifiers `.scaled::n1::ue8m0` or `.scaled::n2::ue8m0` can be used to specify that the instruction uses scale-factor with 1 scale value or 2 packed scale values of `ue8m0` type. Operand `scale-factor` must be used with either of the qualifiers `.scaled::n1::ue8m0` or `.scaled::n2::ue8m0`. Operand `scale-factor`:

  * has type `.b16` when used with qualifier `.scaled::n2::ue8m0` qualifier. Default value of scale-factor is `0x7f7f` in this case if not specified explicitly.

  * has type `.b8` when used with qualifier `.scaled::n1::ue8m0` qualifier. Default value of scale-factor is `0x7f` in this case if not specified explicitly.
...
Rounding modifier is mandatory in all of the following cases:
  ...
  * All conversions involving `.f16x2`, `.e4m3x2, .e5m2x2,`, `.ue5m3x2`, `.bf16x2`, `.tf32`, `.e2m1x2`, `.e2m3x2`, `.e3m2x2`, `.e4m3x4`, `.e5m2x4`, `.e2m1x4`, `.e2m3x4`, `.e3m2x4`, `.s2f6x2` and `.ue8m0x2` instruction types.
...
`.satfinite` modifier is only supported for conversions involving the following types:
  ...
  * `.f16`, `.bf16`, `.f16x2`, `.bf16x2`, `.tf32`, `.ue8m0x2`, `.ue5m3x2` as destination types.

The `.pzo` qualifier is only supported for float-to-float conversions where the destination type is strictly smaller than the source type. When this qualifier is specified, it enforces the generation of positive zeros only in the output. In other words, if the result of the conversion produces `-0.0`, it will be forced to `+0.0`. Note that the `.pzo` qualifier applies only to the results of the data format conversion. In other words, `.pzo` qualifier has no impact on other additional operands such as `rbits`.
...
if (/* inst type is .f16x2 or .bf16x2 */) {
    d[31:16] = convert(a);
    d[15:0]  = convert(b);
} else if (/* inst destination type is .e5m2x2 or .e4m3x2 or .ue8m0x2 or .ue5m3x2 */) {
    if (/* inst source type is .f32 */) {
        d[15:8] = convert(a / scale-factor[7:0]); // scale-factor if present
        d[7:0]  = convert(b / scale-factor[7:0]); // scale-factor if present
    } else {
        d[15:8] = convert(a[31:16] / scale-factor[7:0]); // scale-factor if present
        d[7:0]  = convert(a[15:0] / scale-factor[7:0]); // scale-factor if present
    }
    ...
    if (/* inst source type is .f32 */) {
        d[7:4] = convert(a / scale-factor[7:0]); // scale-factor if present
        d[3:0] = convert(b / scale-factor[7:0]); // scale-factor if present
    } else {
        d[7:4] = convert(a[31:16] / scale-factor[7:0]); // scale-factor if present
        d[3:0]  = convert(a[15:0] / scale-factor[7:0]); // scale-factor if present
    }
} else if (/* inst destination type is .e2m3x2 or .e3m2x2 */) {
    if (/* inst source type is .f32 */) {
        d[15:14] = 0;
        d[13:8] = convert(a / scale-factor[7:0]); // scale-factor if present
        d[7:6] = 0;
        d[5:0] = convert(b / scale-factor[7:0]); // scale-factor if present
    } else {
        d[15:14] = 0;
        d[13:8] = convert(a[31:16] / scale-factor[7:0]); // scale-factor if present
        d[7:6] = 0;
        d[5:0] = convert(a[15:0] / scale-factor[7:0]); // scale-factor if present
    }
    ...
    d[7:6] = 0;
    d[5:0] = convert(f);
} else if (/* inst source type is .e4m3x2 or .e5m2x2 or .e3m2x2 or .e2m3x2 or .ue5m3x2 */) {
...
// Random bits `rbits` semantics for `.rs` rounding:
  ...
  1. Destination type `.f16`: Refer [Figure 39](#cvt-rs-rbits-layout-f16) for random bits layout details.

<a id="cvt-rs-rbits-layout-f16"></a>
...
Figure 39 Random bits layout for `.rs` rounding with `.f16` destination type

  2. Destination type `.bf16`: Refer [Figure 40](#cvt-rs-rbits-layout-bf16) for random bits layout details.

<a id="cvt-rs-rbits-layout-bf16"></a>
...
Figure 40 Random bits layout for `.rs` rounding with `.bf16` destination type

  3. Destination type `.e2m1x4`: Refer [Figure 41](#cvt-rs-rbits-layout-f4) for random bits layout details.

<a id="cvt-rs-rbits-layout-f4"></a>
...
Figure 41 Random bits layout for `.rs` rounding with `.e2m1x4` destination type

  4. Destination type `.e5m2x4`, `.e4m3x4`, `.e3m2x4`, `.e2m3x4`: Refer [Figure 42](#cvt-rs-rbits-layout-f8-f6) for random bits layout details.

<a id="cvt-rs-rbits-layout-f8-f6"></a>
...
Figure 42 Random bits layout for `.rs` rounding with `.e5m2x4`/`.e4m3x4`/`.e3m2x4`/`.e2m3x4` destination type
...
When qualifier `.scaled::n1::ue8m0` is specified, subnormal values for inputs `a` and `b` are flushed to zero before conversion.
...
For `.f16`, `.f16x2`, `.bf16`, `.bf16x2`, `.e4m3x2`, `.e5m2x2`, `.ue5m3x2`, `.ue8m0x2`, `.e4m3x4`, `.e5m2x4` and `.tf32` destination formats, if the input value is `NaN`, then the result is `NaN` in the specified destination format. For `.e2m1x2`, `.e2m3x2`, `.e3m2x2`, `.e2m1x4`, `.e2m3x4`, `.e3m2x4`, `.s2f6x2` destination formats `NaN` results are converted to positive _MAX_NORM_. If the absolute value of input (ignoring sign) is greater than _MAX_NORM_ of the specified destination format, then the result is sign-preserved _MAX_NORM_ of the destination format and a positive _MAX_NORM_ in `.ue8m0x2` and `.ue5m3x2` for which the destination sign is not supported.

Notes

A source register wider than the specified type may be used, except when the source operand has `.bf16` or `.bf16x2` format. The lower `n` bits corresponding to the instruction-type width are used in the conversion. See [Operand Size Exceeding Instruction-Type Size](9.4.1-operand-size-exceeding-instruction-type-size.md#operand-size-exceeding-instruction-type-size) for a description of these relaxed type-checking rules.
...
`.rz` rounding mode on `cvt.{e4m3x2, e5m2x2, e2m3x2, e3m2x2, e2m1x2}.{f32/f16x2/bf16x2}` introduced in PTX ISA version 9.4.

`.pzo` qualifier is introduced in PTX ISA version 9.4.

`.scaled::n1::ue8m0` qualifier is introduced in PTX ISA version 9.4.

`cvt` with `.ue5m3x2` instruction type introduced in PTX ISA version 9.4.
...
`.rz` rounding mode on `cvt.{e4m3x2, e5m2x2, e2m3x2, e3m2x2, e2m1x2}.{f32/f16x2/bf16x2}` is supported on following family-specific architectures:

  * `sm_107f` or higher in the same family

`.pzo` qualifier is supported on following family-specific architectures:

  * `sm_107f` or higher in the same family

`.scaled::n1::ue8m0` qualifier is supported on following family-specific architectures:

  * `sm_107f` or higher in the same family

`cvt` with `.ue5m3x2` instruction type is supported on following family-specific architectures:
...
cvt.rn.satfinite.relu.e2m1x2.f16x2  d, a; // unpack a and convert to two .e2m1 values
cvt.rn.satfinite.e2m3x2.bf16x2  d, a; // unpack a and convert to two .e2m3 values
...
cvt.rz.satfinite.e2m3x2.f32 d, a, b;  // convert a, b to .e2m3 and pack as .e2m3x2 with round towards zero mode

cvt.rn.bf16.f32.pzo          b, f;        // convert fp32 to bf16 with positive zero's only

cvt.rn.relu.satfinite.f16x2.f32.pzo    b1, f, f1;   // convert two fp32 values to packed fp16 outputs with .relu saturation on each output and produce positive zero's only

cvt.rn.satfinite.scaled::n1::ue8m0.e4m3x2.bf16x2 b16_0, b32_0, b8_0;
