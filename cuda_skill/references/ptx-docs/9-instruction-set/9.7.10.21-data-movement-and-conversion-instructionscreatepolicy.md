---
title: "9.7.10.21. Data Movement and Conversion Instructions: createpolicy"
section: "9.7.10.21"
version: "9.4"
...
<a id="data-movement-and-conversion-instructions-createpolicy"></a>

<a id="id279"></a>

####  9.7.10.21. Data Movement and Conversion Instructions: `createpolicy`
...
// Fabric range-based policy
createpolicy.range.fabric.level::primary_priority{.level::secondary_priority}.b64
                                   cache_policy, [aLe, aOff], primary-size, total-size;

// Fraction-based policy
createpolicy.fractional.level::primary_priority{.level::secondary_priority}.b64
                                   cache_policy{, fraction};

// Converting the access property from CUDA APIs
createpolicy.cvt.L2.b64            cache_policy, access-property;
...
The cache eviction policy created using `createpolicy.range.fabric` specifies the cache eviction behaviors for the following three address ranges within a fabric handle:

    * `[aOff .. aOff + (primary-size - 1)]` referred to as primary range.

    * `[aOff + primary-size .. aOff + (total-size - 1)]` referred to as trailing secondary range.

    * `[aOff - (total-size - primary-size) .. (aOff - 1)]` referred to as preceding secondary range.
...
When a range-based cache eviction policy is used in a memory operation with `.level::cache_hint` qualifier, the eviction priorities are applied as follows:
    ...
    * If the memory address falls in either of the secondary ranges, the eviction priority specified by `.level::secondary_priority` is applied.

    * If the memory address does not fall in any of the above ranges, then the applied eviction priority is unspecified.
...
`createpolicy.range.fabric` introduced in PTX ISA version 9.4.

Target ISA Notes

Requires `sm_80` or higher.

`createpolicy.range.fabric` requires `sm_100` or higher.
...
createpolicy.range.fabric.L2::evict_last.L2::evict_first.b64
                                            policy, [leid, off], 0x100000, 0x200000;
