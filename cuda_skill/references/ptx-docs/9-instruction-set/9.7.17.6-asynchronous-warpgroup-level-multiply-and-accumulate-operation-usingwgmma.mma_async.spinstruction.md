---
title: "9.7.17.6. Asynchronous Warpgroup Level Multiply-and-Accumulate Operation using wgmma.mma_async.sp instruction"
section: "9.7.17.6"
version: "9.4"
...
<a id="asynchronous-warpgroup-level-matrix-instructions-for-sparse-wgmma"></a>

<a id="id464"></a>

####  9.7.17.6. Asynchronous Warpgroup Level Multiply-and-Accumulate Operation using `wgmma.mma_async.sp` instruction
...
<a id="asynchronous-warpgroup-level-sparse-matrix-storage"></a>

<a id="id465"></a>

#####  9.7.17.6.1. Sparse matrix storage
...
Matrix A and its corresponding input operand to the sparse wgmma is similar to the diagram shown in [Figure 114](9.7.16.6-matrix-multiply-accumulate-operation-usingmma.spinstruction-with-sparse-matrix-a.md#sparse-mma-storage-example), with an appropriate matrix size.
...
<a id="f16-metadata-example-wgmma"></a>

![_images/f16-metadata-example.png](https://docs.nvidia.com/cuda/parallel-thread-execution/_images/f16-metadata-example.png)

Figure 174 Sparse WGMMA metadata example for `.f16`/`.bf16` type.

The sparsity selector indicates a thread-pair within a group of four consecutive threads which contributes the sparsity metadata. Hence, the sparsity selector must be either 0 (threads T0, T1) or 1 (threads T2, T3); any other value results in undefined behavior.
...
<a id="e4m3-e5m2-metadata-example-wgmma"></a>

![_images/u8s8-metadata-example.png](https://docs.nvidia.com/cuda/parallel-thread-execution/_images/u8s8-metadata-example.png)

Figure 176 Sparse WGMMA metadata example for `.e4m3`/`.e5m2` type.

All threads contribute the sparsity metadata and the sparsity selector must be 0; any other value results in undefined behavior.
...
<a id="u8s8-metadata-example-wgmma"></a>

![_images/u8s8-metadata-example.png](https://docs.nvidia.com/cuda/parallel-thread-execution/_images/u8s8-metadata-example.png)

Figure 177 Sparse WGMMA metadata example for `.u8`/`.s8` type.

All threads contribute the sparsity metadata and the sparsity selector must be 0; any other value results in undefined behavior.

<a id="asynchronous-warpgroup-level-matrix-fragments-for-sparse-wgmma"></a>

<a id="id466"></a>

#####  9.7.17.6.2. Matrix fragments for warpgroup-level multiply-accumulate operation with sparse matrix A
...
<a id="asynchronous-warpgroup-level-matrix-fragment-sparse-wgmma-64n32"></a>

<a id="id467"></a>

######  9.7.17.6.2.1. Matrix Fragments for sparse `wgmma.mma_async.m64nNk32`

A warpgroup executing sparse `wgmma.mma_async.m64nNk32` will compute an MMA operation of shape `.m64nNk32` where N is a valid n dimension as listed in [Matrix Shape](9.7.17.2-matrix-shape.md#asynchronous-warpgroup-level-matrix-shape).
...
Elements of the matrix are distributed across the threads in a warpgroup so each thread of the warpgroup holds a fragment of the matrix.
  ...
  * Multiplicand A, from shared memory is documented in [Shared Memory Matrix Layout](9.7.17.5-asynchronous-warpgroup-level-matrix-multiply-accumulate-operation-usingwgmma.mma_asyncinstruction.md#asynchronous-warpgroup-level-matrix-shared-memory-layout).
...
>
> The layout of the fragments held by different threads is shown in [Figure 178](#sparse-wgmma-64n32-f16-bf16-a).
>
> <a id="sparse-wgmma-64n32-f16-bf16-a"></a>
>
...
>
> Figure 178 Sparse WGMMA .m64nNk32 fragment layout for matrix A with `.f16`/`.bf16` type.

  * Accumulator D:

Matrix fragments for accumulator D are the same as in case of [Matrix Fragments for wgmma.mma_async.m64nNk32](9.7.17.5-asynchronous-warpgroup-level-matrix-multiply-accumulate-operation-usingwgmma.mma_asyncinstruction.md#asynchronous-warpgroup-level-matrix-register-fragment-wgmma-64n32) for the same `.dtype` format.

  * Multiplicand B:

Shared memory layout for Matrix B is documented in [Shared Memory Matrix Layout](9.7.17.5-asynchronous-warpgroup-level-matrix-multiply-accumulate-operation-usingwgmma.mma_asyncinstruction.md#asynchronous-warpgroup-level-matrix-shared-memory-layout).
...
[Figure 179](#sparse-wgmma-metadata-64n32-f16bf16) shows the mapping of the metadata bits to the elements of matrix A for a warp. In this figure, variable `i` represents the value of the sparsity selector operand.

> <a id="sparse-wgmma-metadata-64n32-f16bf16"></a>
>
...
>
> Figure 179 Sparse WGMMA .m64nNk32 metadata layout for `.f16`/`.bf16` type.

<a id="asynchronous-warpgroup-level-matrix-fragment-sparse-wgmma-64n16"></a>

<a id="id468"></a>

######  9.7.17.6.2.2. Matrix Fragments for sparse `wgmma.mma_async.m64nNk16`

A warpgroup executing sparse `wgmma.mma_async.m64nNk16` will compute an MMA operation of shape `.m64nNk16` where N is a valid n dimension as listed in [Matrix Shape](9.7.17.2-matrix-shape.md#asynchronous-warpgroup-level-matrix-shape).
...
>
> The layout of the fragments held by different threads is shown in [Figure 180](#sparse-wgmma-64n16-tf32-a).
>
> <a id="sparse-wgmma-64n16-tf32-a"></a>
>
...
>
> Figure 180 Sparse WGMMA .m64nNk16 fragment layout for matrix A with `.tf32` type.

  * Accumulator D:

Matrix fragments for accumulator D are the same as in case of [Matrix Fragments for wgmma.mma_async.m64nNk8](9.7.17.5-asynchronous-warpgroup-level-matrix-multiply-accumulate-operation-usingwgmma.mma_asyncinstruction.md#asynchronous-warpgroup-level-matrix-register-fragment-wgmma-64n8) for the same `.dtype` format.
...
[Figure 181](#sparse-wgmma-metadata-64n16-tf32) shows the mapping of the metadata bits to the elements of matrix A for a warp. In this figure, variable `i` represents the value of the sparsity selector operand.

> <a id="sparse-wgmma-metadata-64n16-tf32"></a>
>
...
>
> Figure 181 Sparse WGMMA .m64nNk16 metadata layout for `.tf32` type.

<a id="asynchronous-warpgroup-level-matrix-fragment-sparse-wgmma-64n64"></a>

<a id="id469"></a>

######  9.7.17.6.2.3. Matrix Fragments for sparse `wgmma.mma_async.m64nNk64`

A warpgroup executing sparse `wgmma.mma_async.m64nNk64` will compute an MMA operation of shape `.m64nNk64` where N is a valid n dimension as listed in [Matrix Shape](9.7.17.2-matrix-shape.md#asynchronous-warpgroup-level-matrix-shape).
...
<div>
<div>
<code><span>.e4m3</span></code> /</div>
<div><code><span>.e5m2</span></code></div>
</div>
</td>
<td>
<div>
<div>A vector expression containing four <code><span>.b32</span></code>
</div>
<div>registers, with each register containing four</div>
<div>non-zero <code><span>.e4m3</span></code> /<code><span>.e5m2</span></code> elements out of</div>
<div>eight consecutive elements from matrix A.</div>
</div>
</td>
<td rowspan="2">
<div>
<div><br/></div>
<div>Non-zero elements:</div>
<div>a0, a1, a2, … , a15</div>
<div><br/></div>
<div>Mapping of the non-zero</div>
<div>elements is as described in</div>
<div><a href="#asynchronous-warpgroup-level-sparse-matrix-storage"><span>Sparse matrix storage</span></a></div>
</div>
</td>
</tr>
<tr>
...
<div>
<div>
<code><span>.s8</span></code> /</div>
<div><code><span>.u8</span></code></div>
</div>
</td>
<td>
<div>
...
<div>registers, with each register containing four</div>
<div>non-zero <code><span>.s8</span></code> /<code><span>.u8</span></code> elements out of</div>
<div>eight consecutive elements from matrix A.</div>
</div>
</td>
</tr>
...
>
> The layout of the fragments held by different threads is shown in [Figure 182](#sparse-wgmma-64n64-e4m3-e5m2-s8-u8-a).
>
> <a id="sparse-wgmma-64n64-e4m3-e5m2-s8-u8-a"></a>
>
...
>
> Figure 182 Sparse WGMMA .m64nNk64 fragment layout for matrix A with `.e4m3`/ `.e5m2`/ `.s8`/ `.u8` type.
...
[Figure 183](#sparse-wgmma-metadata-64n64-e4m3-e5m2-s8-u8-first32col) shows the mapping of the metadata bits to the elements of columns 0–31 of matrix A.

> <a id="sparse-wgmma-metadata-64n64-e4m3-e5m2-s8-u8-first32col"></a>
>
...
>
> Figure 183 Sparse WGMMA .m64nNk64 metadata layout for `.e4m3`/ `.e5m2`/ `.s8`/ `.u8` type for columns 0–31

[Figure 184](#sparse-wgmma-metadata-64n64-e4m3-e5m2-s8-u8-last32col) shows the mapping of the metadata bits to the elements of columns 32–63 of matrix A.

> <a id="sparse-wgmma-metadata-64n64-e4m3-e5m2-s8-u8-last32col"></a>
>
...
>
> Figure 184 Sparse WGMMA .m64nNk64 metadata layout for `.e4m3`/ `.e5m2`/ `.s8`/ `.u8` type for columns 32–63

<a id="asynchronous-warpgroup-level-matrix-instructions-wgmma-mma-sp"></a>

<a id="id470"></a>

#####  9.7.17.6.3. Asynchronous Multiply-and-Accumulate Instruction: `wgmma.mma_async.sp`
