---
title: "9.7.16.6. Matrix multiply-accumulate operation using mma.sp instruction with sparse matrix A"
section: "9.7.16.6"
version: "9.4"
url: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#warp-level-matrix-instructions-for-sparse-mma
---

<a id="warp-level-matrix-instructions-for-sparse-mma"></a>

<a id="id433"></a>

####  9.7.16.6. Matrix multiply-accumulate operation using `mma.sp` instruction with sparse matrix A
...
<a id="warp-level-sparse-matrix-storage"></a>
...
[Figure 114](#sparse-mma-storage-example) shows an example of a 16x16 matrix A represented in sparse format and sparsity selector indicating which thread in a group of four consecutive threads stores the metadata.

<a id="sparse-mma-storage-example"></a>
...
Figure 114 Sparse MMA storage example
...
The sparsity selector indicates the threads which contribute metadata as listed below:
  ...
  * `m16n8k32`: A thread-pair within a group of four consecutive threads contributes the sparsity metadata. Hence, the sparsity selector must be either 0 (threads T0, T1) or 1 (threads T2, T3); any other value results in undefined behavior.

Sparse `mma.sp{::ordered_metadata}` with `.tf32` type

When matrix A has `.tf32` elements, matrix A is structured sparse at a granularity of 1:2. In other words, each chunk of two adjacent elements in a row of matrix A has one zero and one non-zero element. Only the non-zero elements are stored in the operand for matrix A and their positions in a two-wide chunk in matrix A are indicated by the 4-bit index in the metadata. `0b1110` and `0b0100` are the only meaningful index values; any other values result in undefined behavior.

<a id="tf32-metadata-example"></a>

![_images/tf32-metadata-example.png](https://docs.nvidia.com/cuda/parallel-thread-execution/_images/tf32-metadata-example.png)

Figure 116 Sparse MMA metadata example for `.tf32` type.
...
When matrices A and B have `.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` elements, matrix A is structured sparse at a granularity of 2:4. In other words, each chunk of four adjacent elements in a row of matrix A have two zeroes and two non-zero elements. Only the two non-zero elements are stored in sparse matrix and their positions in the four-wide chunk are indicated by two 2-bit indices in the metadata. `0b0100`, `0b1000`, `0b1001`, `0b1100`, `0b1101`, `0b1110` are the meaningful values of indices; any other values result in undefined behavior.

<a id="fp8-metadata-example"></a>

![_images/fp8-metadata-example.png](https://docs.nvidia.com/cuda/parallel-thread-execution/_images/fp8-metadata-example.png)

Figure 119 Sparse MMA metadata example for `.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.

The sparsity selector indicates the threads which contribute metadata as listed below:

  * `m16n8k64`: All threads within a group of four consecutive threads contribute the sparsity metadata. Hence, the sparsity selector in this case must be 0. Any other value of sparsity selector results in undefined behavior.
...
<a id="fp4-metadata-example"></a>

![_images/fp4-metadata-example.png](https://docs.nvidia.com/cuda/parallel-thread-execution/_images/fp4-metadata-example.png)

Figure 120 Sparse MMA metadata example for `.e2m1` type with `.kind::mxf4` or `.kind::mxf4nvf4`

The sparsity selector indicates the threads which contribute metadata as listed below:

  * `m16n8k128`: All threads within a group of four consecutive threads contribute the sparsity metadata. Hence, the sparsity selector in this case must be 0. Any other value of sparsity selector results in undefined behavior.

<a id="warp-level-matrix-fragments-for-sparse-mma"></a>

<a id="id435"></a>

#####  9.7.16.6.2. Matrix fragments for multiply-accumulate operation with sparse matrix A
...
<a id="warp-level-matrix-fragment-sparse-mma-16816-f16bf16"></a>

<a id="id436"></a>

######  9.7.16.6.2.1. Matrix Fragments for sparse `mma.m16n8k16` with `.f16` and `.bf16` types
...
The layout of the fragments held by different threads is shown in [Figure 121](#sparse-mma-16816-f16-bf16-a).

<a id="sparse-mma-16816-f16-bf16-a"></a>
...
Figure 121 Sparse MMA .m16n8k16 fragment layout for matrix A with `.f16`/`.bf16` type.
...
```
  ...
  * Matrix fragments for multiplicand B and accumulators C and D are the same as in case of [Matrix Fragments for mma.m16n8k16 with floating point type](9.7.16.5-matrix-multiply-accumulate-operation-usingmmainstruction.md#warp-level-matrix-fragment-mma-16816-float) for `.f16`/`.b16` formats.

  * Metadata: A `.b32` register containing 16 2-bit vectors each storing the index of a non-zero element of a 4-wide chunk of matrix A as shown in [Figure 122](#sparse-mma-metadata-16816-f16bf16).

> <a id="sparse-mma-metadata-16816-f16bf16"></a>
>
...
>
> Figure 122 Sparse MMA .m16n8k16 metadata layout for `.f16`/`.bf16` type.

<a id="warp-level-matrix-fragment-sparse-mma-16832-f16bf16"></a>

<a id="id437"></a>

######  9.7.16.6.2.2. Matrix Fragments for sparse `mma.m16n8k32` with `.f16` and `.bf16` types
...
The layout of the fragments held by different threads is shown in [Figure 123](#sparse-mma-16832-f16-bf16-a).

<a id="sparse-mma-16832-f16-bf16-a"></a>
...
Figure 123 Sparse MMA .m16n8k32 fragment layout for matrix A with `.f16`/`.bf16` type.
...
The layout of the fragments held by different threads is shown in [Figure 124](#sparse-mma-16832-f16bf16-b).

<a id="sparse-mma-16832-f16bf16-b"></a>
...
Figure 124 Sparse MMA .m16n8k32 fragment layout for matrix B with `.f16`/`.bf16` type.

  * Matrix fragments for accumulators C and D are the same as in case of [Matrix Fragments for mma.m16n8k16 with floating point type](9.7.16.5-matrix-multiply-accumulate-operation-usingmmainstruction.md#warp-level-matrix-fragment-mma-16816-float) for `.f16`/`.b16` formats.

  * Metadata: A `.b32` register containing 16 2-bit vectors with each pair of 2-bit vectors storing the indices of two non-zero element from a 4-wide chunk of matrix A as shown in [Figure 125](#sparse-mma-metadata-16832-f16bf16).

> <a id="sparse-mma-metadata-16832-f16bf16"></a>
>
...
>
> Figure 125 Sparse MMA .m16n8k32 metadata layout for `.f16`/`.bf16` type.

<a id="warp-level-matrix-fragment-sparse-mma-16816-tf32"></a>

<a id="id438"></a>

######  9.7.16.6.2.3. Matrix Fragments for sparse `mma.m16n8k16` with `.tf32` floating point type
...
The layout of the fragments held by different threads is shown in [Figure 126](#sparse-mma-16816-tf32-a).

<a id="sparse-mma-16816-tf32-a"></a>
...
Figure 126 Sparse MMA .m16n8k16 fragment layout for matrix A with `.tf32` type.
...
The layout of the fragments held by different threads is shown in [Figure 127](#sparse-mma-16816-tf32-b).

<a id="sparse-mma-16816-tf32-b"></a>
...
Figure 127 Sparse MMA .m16n8k16 fragment layout for matrix B with `.tf32` type.

  * Matrix fragments for accumulators C and D are the same as in case of [Matrix Fragments for mma.m16n8k16 with floating point type](9.7.16.5-matrix-multiply-accumulate-operation-usingmmainstruction.md#warp-level-matrix-fragment-mma-16816-float).

  * Metadata: A `.b32` register containing 8 4-bit vectors each storing the index of a non-zero element of a 2-wide chunk of matrix A as shown in [Figure 128](#sparse-mma-metadata-16816-tf32).

> <a id="sparse-mma-metadata-16816-tf32"></a>
>
...
>
> Figure 128 Sparse MMA .m16n8k16 metadata layout for `.tf32` type.

<a id="warp-level-matrix-fragment-sparse-mma-1688-tf32"></a>

<a id="id439"></a>

######  9.7.16.6.2.4. Matrix Fragments for sparse `mma.m16n8k8` with `.tf32` floating point type
...
The layout of the fragments held by different threads is shown in [Figure 129](#sparse-mma-1688-tf32).

<a id="sparse-mma-1688-tf32"></a>
...
Figure 129 Sparse MMA .m16n8k8 fragment layout for matrix A with `.tf32` type.
...
```
  ...
  * Matrix fragments for multiplicand B and accumulators C and D are the same as in case of [Matrix Fragments for mma.m16n8k8](9.7.16.5-matrix-multiply-accumulate-operation-usingmmainstruction.md#warp-level-matrix-fragment-mma-1688) for `.tf32` format.

  * Metadata: A `.b32` register containing 8 4-bit vectors each storing the index of a non-zero element of a 2-wide chunk of matrix A as shown in [Figure 130](#sparse-mma-metadata-1688-tf32).

> <a id="sparse-mma-metadata-1688-tf32"></a>
>
...
>
> Figure 130 Sparse MMA .m16n8k8 metadata layout for `.tf32` type.

<a id="warp-level-matrix-fragment-sparse-mma-16832-u8s8"></a>

<a id="id440"></a>

######  9.7.16.6.2.5. Matrix Fragments for sparse `mma.m16n8k32` with `.u8` / `.s8` integer type
...
The layout of the fragments held by different threads is shown in [Figure 131](#sparse-mma-16832-u8s8-a).

<a id="sparse-mma-16832-u8s8-a"></a>
...
Figure 131 Sparse MMA .m16n8k32 fragment layout for matrix A with `.u8`/`.s8` type.
...
```
  ...
  * Matrix fragments for multiplicand B and accumulators C and D are the same as in case of [Matrix Fragments for mma.m16n8k32](9.7.16.5-matrix-multiply-accumulate-operation-usingmmainstruction.md#warp-level-matrix-fragment-mma-16832).

  * Metadata: A `.b32` register containing 16 2-bit vectors with each pair of 2-bit vectors storing the indices of two non-zero elements from a 4-wide chunk of matrix A as shown in [Figure 132](#sparse-mma-metadata-16832-u8s8).

> <a id="sparse-mma-metadata-16832-u8s8"></a>
>
...
>
> Figure 132 Sparse MMA .m16n8k32 metadata layout for `.u8`/`.s8` type.

<a id="warp-level-matrix-fragment-sparse-mma-16864-u8s8-fp8"></a>

<a id="id441"></a>

######  9.7.16.6.2.6. Matrix Fragments for sparse `mma.m16n8k64` with `.u8` / `.s8` / `.e4m3` / `.e5m2` type
...
<tbody>
<tr>
<td><p><code><span>.u8</span></code> / <code><span>.s8</span></code></p></td>
<td><p>A vector expression containing four <code><span>.b32</span></code> registers, with each
register containing four non-zero <code><span>.u8</span></code> / <code><span>.s8</span></code> elements out
of 8 consecutive elements from matrix A.</p></td>
<td rowspan="2"><p>Mapping of the non-zero elements is
as described in <a href="#warp-level-sparse-matrix-storage"><span>Sparse matrix storage</span></a>.</p></td>
</tr>
<tr>
<td><p><code><span>.e4m3</span></code> /
<code><span>.e5m2</span></code> /
<code><span>.e3m2</span></code> /
<code><span>.e2m3</span></code> /
<code><span>.e2m1</span></code></p></td>
<td><p>A vector expression containing four <code><span>.b32</span></code> registers, with each
register containing four non-zero <code><span>.e4m3</span></code> / <code><span>.e5m2</span></code> /
<code><span>.e3m2</span></code> / <code><span>.e2m3</span></code> / <code><span>.e2m1</span></code> elements out of 8 consecutive
elements from matrix A.</p></td>
</tr>
</tbody>
</table>

The layout of the fragments held by different threads is shown in [Figure 133](#sparse-mma-16864-u8s8-a-first32col) and [Figure 134](#sparse-mma-16864-u8s8-a-last32col).

<a id="sparse-mma-16864-u8s8-a-first32col"></a>
...
Figure 133 Sparse MMA .m16n8k64 fragment layout for columns 0–31 of matrix A with `.u8`/`.s8`/`.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.

<a id="sparse-mma-16864-u8s8-a-last32col"></a>
...
Figure 134 Sparse MMA .m16n8k64 fragment layout for columns 32–63 of matrix A with `.u8`/`.s8`/`.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.
...
<tr>
<td><p><code><span>.u8</span></code> / <code><span>.s8</span></code></p></td>
<td><p>A vector expression containing four <code><span>.b32</span></code> registers,
each containing four <code><span>.u8</span></code> / <code><span>.s8</span></code> elements from
matrix B.</p></td>
<td rowspan="2"><p>b0, b1, b2, b3, …, b15</p></td>
</tr>
<tr>
<td><p><code><span>.e4m3</span></code> /
<code><span>.e5m2</span></code> /
...
<code><span>.e2m1</span></code></p></td>
<td><p>A vector expression containing four <code><span>.b32</span></code> registers,
each containing four <code><span>.e4m3</span></code> / <code><span>.e5m2</span></code> / <code><span>.e3m2</span></code> /
<code><span>.e2m3</span></code> / <code><span>.e2m1</span></code> elements from matrix B.</p></td>
</tr>
</tbody>
</table>

The layout of the fragments held by different threads is shown in [Figure 135](#sparse-mma-16864-u8s8-b1), [Figure 136](#sparse-mma-16864-u8s8-b2), [Figure 137](#sparse-mma-16864-u8s8-b3) and [Figure 138](#sparse-mma-16864-u8s8-b4).

<a id="sparse-mma-16864-u8s8-b1"></a>
...
Figure 135 Sparse MMA .m16n8k64 fragment layout for rows 0–15 of matrix B with `.u8`/`.s8`/`.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.

<a id="sparse-mma-16864-u8s8-b2"></a>
...
Figure 136 Sparse MMA .m16n8k64 fragment layout for rows 16–31 of matrix B with `.u8`/`.s8`/`.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.

<a id="sparse-mma-16864-u8s8-b3"></a>
...
Figure 137 Sparse MMA .m16n8k64 fragment layout for rows 32–47 of matrix B with `.u8`/`.s8`/`.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.

<a id="sparse-mma-16864-u8s8-b4"></a>
...
Figure 138 Sparse MMA .m16n8k64 fragment layout for rows 48–63 of matrix B with `.u8`/`.s8`/`.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.

  * Matrix fragments for accumulators C and D are the same as in case of [Matrix Fragments for mma.m16n8k16 with integer type](9.7.16.5-matrix-multiply-accumulate-operation-usingmmainstruction.md#warp-level-matrix-fragment-mma-16816-i8-f8).

  * Metadata: A `.b32` register containing 16 2-bit vectors with each pair of 2-bit vectors storing the indices of two non-zero elements from a 4-wide chunk of matrix A as shown in [Figure 139](#sparse-mma-metadata-16864-u8s8-first32col) and [Figure 140](#sparse-mma-metadata-16864-u8s8-last32col).

> <a id="sparse-mma-metadata-16864-u8s8-first32col"></a>
>
...
>
> Figure 139 Sparse MMA .m16n8k64 metadata layout for columns 0–31 for `.u8`/`.s8`/`.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.
>
> <a id="sparse-mma-metadata-16864-u8s8-last32col"></a>
>
...
>
> Figure 140 Sparse MMA .m16n8k64 metadata layout for columns 32–63 for `.u8`/`.s8`/`.e4m3`/`.e5m2`/`.e3m2`/`.e2m3`/`.e2m1` type.

<a id="warp-level-matrix-fragment-sparse-mma-16864-u4s4"></a>

<a id="id442"></a>

######  9.7.16.6.2.7. Matrix Fragments for sparse `mma.m16n8k64` with `.u4` / `.s4` integer type
...
The layout of the fragments held by different threads is shown in [Figure 141](#sparse-mma-16864-u4s4-a).

<a id="sparse-mma-16864-u4s4-a"></a>
...
Figure 141 Sparse MMA .m16n8k64 fragment layout for matrix A with `.u4`/`.s4` type.
...
```
  ...
  * Matrix fragments for multiplicand B and accumulators C and D are the same as in case of [Matrix Fragments for mma.m16n8k64](9.7.16.5-matrix-multiply-accumulate-operation-usingmmainstruction.md#warp-level-matrix-fragment-mma-16864).

  * Metadata: A `.b32` register containing 16 2-bit vectors with each pair of 2-bit vectors storing the indices of four non-zero elements from a 8-wide chunk of matrix A as shown in [Figure 142](#sparse-mma-metadata-16864-u4s4).

> <a id="sparse-mma-metadata-16864-u4s4"></a>
>
...
>
> Figure 142 Sparse MMA .m16n8k64 metadata layout for `.u4`/`.s4` type.

<a id="warp-level-matrix-fragment-sparse-mma-168128-u4s4"></a>

<a id="id443"></a>

######  9.7.16.6.2.8. Matrix Fragments for sparse `mma.m16n8k128` with `.u4` / `.s4` integer type
...
<tbody>
<tr>
<td><p><code><span>.u4</span></code> / <code><span>.s4</span></code></p></td>
<td><p>A vector expression containing four <code><span>.b32</span></code> registers, with each
register containing eight non-zero <code><span>.u4</span></code> / <code><span>.s4</span></code> elements out
...
<td><p>A vector expression containing four <code><span>.b32</span></code> registers, with each
register containing eight non-zero <code><span>.e2m1</span></code> elements out
of 16 consecutive elements from matrix A.</p></td>
</tr>
</tbody>
</table>

The layout of the fragments held by different threads is shown in [Figure 143](#sparse-mma-168128-u4s4-a-first64col) and [Figure 144](#sparse-mma-168128-u4s4-a-last64col).

<a id="sparse-mma-168128-u4s4-a-first64col"></a>
...
Figure 143 Sparse MMA .m16n8k128 fragment layout for columns 0–63 of matrix A with `.u4`/`.s4`/`.e2m1` type.

<a id="sparse-mma-168128-u4s4-a-last64col"></a>
...
Figure 144 Sparse MMA .m16n8k128 fragment layout for columns 64–127 of matrix A with `.u4`/`.s4`/`.e2m1` type.
...
<tr>
<td><p><code><span>.u4</span></code> / <code><span>.s4</span></code></p></td>
<td><p>A vector expression containing four <code><span>.b32</span></code> registers, each containing
eight <code><span>.u4</span></code> / <code><span>.s4</span></code> elements from matrix B.</p></td>
<td rowspan="2"><p>b0, b1, b2, b3, …, b31</p></td>
</tr>
<tr>
<td><p><code><span>.e2m1</span></code></p></td>
<td><p>A vector expression containing four <code><span>.b32</span></code> registers, each containing
eight <code><span>.e2m1</span></code> elements from matrix B.</p></td>
</tr>
</tbody>
</table>

The layout of the fragments held by different threads is shown in [Figure 145](#sparse-mma-168128-u4s4-b1), [Figure 146](#sparse-mma-168128-u4s4-b2), [Figure 147](#sparse-mma-168128-u4s4-b3), [Figure 148](#sparse-mma-168128-u4s4-b4).

<a id="sparse-mma-168128-u4s4-b1"></a>
...
Figure 145 Sparse MMA .m16n8k128 fragment layout for rows 0–31 of matrix B with `.u4`/`.s4`/`.e2m1` type.

<a id="sparse-mma-168128-u4s4-b2"></a>
...
Figure 146 Sparse MMA .m16n8k128 fragment layout for rows 32–63 of matrix B with `.u4`/`.s4`/`.e2m1` type.

<a id="sparse-mma-168128-u4s4-b3"></a>
...
Figure 147 Sparse MMA .m16n8k128 fragment layout for rows 64–95 of matrix B with `.u4`/`.s4`/`.e2m1` type.

<a id="sparse-mma-168128-u4s4-b4"></a>
...
Figure 148 Sparse MMA .m16n8k128 fragment layout for rows 96–127 of matrix B with `.u4`/`.s4`/`.e2m1` type.

  * Matrix fragments for accumulators C and D are the same as in case of [Matrix Fragments for mma.m16n8k64](9.7.16.5-matrix-multiply-accumulate-operation-usingmmainstruction.md#warp-level-matrix-fragment-mma-16864).

  * Metadata: A `.b32` register containing 16 2-bit vectors with each pair of 2-bit vectors storing the indices of four non-zero elements from a 8-wide chunk of matrix A as shown in [Figure 149](#sparse-mma-metadata-168128-u4s4-first64col) and [Figure 150](#sparse-mma-metadata-168128-u4s4-last64col).

> <a id="sparse-mma-metadata-168128-u4s4-first64col"></a>
>
...
>
> Figure 149 Sparse MMA .m16n8k128 metadata layout for columns 0–63 for `.u4`/`.s4`/`.e2m1` type.
>
> <a id="sparse-mma-metadata-168128-u4s4-last64col"></a>
>
...
>
> Figure 150 Sparse MMA .m16n8k128 metadata layout for columns 64–127 for `.u4`/`.s4`/`.e2m1` type.

<a id="warp-level-matrix-instructions-sparse-mma"></a>

<a id="id444"></a>

#####  9.7.16.6.3. Multiply-and-Accumulate Instruction: `mma.sp` / `mma.sp::ordered_metadata`
