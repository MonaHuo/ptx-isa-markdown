...
Load register variable `d` from the location specified by the source address operand `a` in specified state space. If no state space is given, perform the load using [Generic Addressing](../6-instruction-operands/6.4.1.1-generic-addressing.md#generic-addressing).

If no sub-qualifier is specified with `.shared` state space, then `::cta` is assumed by default.

Supported addressing modes for operand `a` and alignment requirements are described in [Addresses as Operands](../6-instruction-operands/6.4.1-addresses-as-operands.md#addresses-as-operands)
...
Instruction `ld.param{::func}` used for reading value returned from device function call cannot be predicated. See [Parameter State Space](../5-state-spaces-types-and-variables/5.1.6-parameter-state-space.md#parameter-state-space) and [Function Declarations and Definitions](../7-abstracting-the-abi/7.1-function-declarations-and-definitions.md#function-declarations-and-definitions) for descriptions of the proper use of `ld.param`.

The `.relaxed` and `.acquire` qualifiers indicate memory synchronization as described in the [Memory Consistency Model](../8-memory-consistency-model/8-memory-consistency-model.md#memory-consistency-model). The `.scope` qualifier indicates the set of threads with which an `ld.relaxed` or `ld.acquire` instruction can directly synchronize1. The `.weak` qualifier indicates a memory instruction with no synchronization. The effects of this instruction become visible to other threads only when synchronization is established by other means.

The optional qualifier `.proxy::readonly` may be used to indicate that the memory operation is performed using the readonly proxy. When this qualifier is specified, the memory location referred to by source operand `a` is treated as read-only during the lifetime of the kernel. The `proxy::readonly` qualifier may only be used with `global` state space or with generic addressing where the address points to `global` or `const` state space. If the generic address does not fall within the address window of the `global` or `const` memory, then the behavior is undefined.

The semantic details of `.mmio` qualifier are described in the [Memory Consistency Model](../8-memory-consistency-model/8-memory-consistency-model.md#memory-consistency-model). Only `.sys` thread scope is valid for `ld.mmio` operation.

The semantic details of `.volatile` qualifier are described in the [Memory Consistency Model](../8-memory-consistency-model/8-memory-consistency-model.md#memory-consistency-model).
...
Support for `.proxy::readonly` qualifier introduced in PTX ISA version 9.4.
...
Support for `.proxy::readonly` qualifier requires `sm_90` or higher.
...
createpolicy.fractional.L2::evict_last.L2::evict_unchanged.b64 cache_policy, 1.0;
ld.global.L2::cache_hint.b64  x, [p], cache_policy;
ld.param::entry.b32 %rp1, [kparam1];

ld.global.b128   %r0, [gbl];   // 128-bit load

ld.global.acquire.mmio.sys.u32 %r0, [gbl];
...
ld.global.u32.proxy::readonly %r0, [%rd0];
ld.u32.proxy::readonly %r1, [%rd1];
```
