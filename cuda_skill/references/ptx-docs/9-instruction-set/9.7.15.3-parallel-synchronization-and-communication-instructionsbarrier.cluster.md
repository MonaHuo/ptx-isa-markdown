---
title: "9.7.15.3. Parallel Synchronization and Communication Instructions: barrier.cluster"
section: "9.7.15.3"
version: "9.4"
...
<a id="parallel-synchronization-and-communication-instructions-barrier-cluster"></a>
...
The optional `.sem` and `.acquire` qualifiers on instructions `barrier.cluster.arrive` and `barrier.cluster.wait` specify the memory synchronization as described in the [Memory Consistency Model](../8-memory-consistency-model/8-memory-consistency-model.md#memory-consistency-model). If the optional `.sem` qualifier is absent for `barrier.cluster.arrive`, `.release` is assumed by default. If the optional `.acquire` qualifier is absent for `barrier.cluster.wait`, `.acquire` is assumed by default.
