---
title: "11.4.7. Performance-Tuning Directives: .pragma"
section: "11.4.7"
version: "9.4"
url: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#performance-tuning-directives-pragma
---

<a id="performance-tuning-directives-pragma"></a>

<a id="id671"></a>

###  11.4.7. Performance-Tuning Directives: `.pragma`

`.pragma`

Pass directives to PTX backend compiler.

Syntax

```ptx
.pragma list-of-strings ;
```

Description

Pass module-scoped, entry-scoped, or statement-level directives to the PTX backend compiler.

The `.pragma` directive may occur at module-scope, at entry-scope, or at statement-level.

Semantics

The interpretation of `.pragma` directive strings is implementation-specific and has no impact on PTX semantics. See [Descriptions of .pragma Strings](../12-descriptions-of.pragmastrings/12-descriptions-of.pragmastrings.md#descriptions-pragma-strings) for descriptions of the pragma strings defined in `ptxas`.

PTX ISA Notes

Introduced in PTX ISA version 2.0.

Target ISA Notes

Supported on all target architectures.

Examples

```ptx
.pragma "nounroll";    // disable unrolling in backend

// disable unrolling for current kernel
.entry foo .pragma "nounroll"; { ... }
```
