...
Source: https://docs.nvidia.com/cuda/cuda-driver-api/cuda_driver_api/group__CUDA__EGL.html

#  6.12. EGL Interoperability
...
Parameters
  ...
  * **pStream** – - CUDA stream for synchronization and any data migrations implied by CUeglResourceLocationFlags.

  * **timeout** – - Desired timeout in usec for a new frame to be acquired. If set as CUDA_EGL_INFINITE_TIMEOUT, acquire waits infinitely. After timeout occurs CUDA consumer tries to acquire an old frame if available and EGL_SUPPORT_REUSE_NV flag is set.
...
Parameters
  ...
  * **eglframe** – - CUDA Eglstream Proucer Frame handle returned from the consumer over EglStream.
