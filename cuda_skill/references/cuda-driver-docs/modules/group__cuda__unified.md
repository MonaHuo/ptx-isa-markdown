...
Source: https://docs.nvidia.com/cuda/cuda-driver-api/cuda_driver_api/group__CUDA__UNIFIED.html

#  6.44. Unified Addressing
...
##  6.44.3. Looking Up Information from Pointer Values
...
##  6.44.4. Automatic Mapping of Host Allocated Host Memory
...
##  6.44.5. Automatic Registration of Peer Memory
...
##  6.44.6. Exceptions, Disjoint Addressing
...
`` CUresult cuMemGetLocationInfo(CUdeviceptr ptr, size_t size, size_t summaryGranularity, size_t samplingGranularity, CUmemLocation *location_out) ``

Gets residency information for a memory address range.

Retrieves memory location information for the specified address range starting at `ptr` with size `size`. The API summarizes the location information with a granularity specified by `summaryGranularity`. For each summary region, the API determines the most common location by sampling memory at intervals defined by `samplingGranularity` within that region.
...
For example, with an address range of 1GB, a `summaryGranularity` of 128MB, and a `samplingGranularity` of 2MB, the function will:
  ...
  * Populate the `location_out` array with 8 entries, one for each 128MB region `summaryGranularity` should be less than or equal to `size` and greater than 0. `samplingGranularity` should be less than or equal to `summaryGranularity`. If the `samplingGranularity` is set to 0, a system dependent value is used as the granularity. In all other cases, the call returns CUDA_ERROR_INVALID_VALUE.

When the memory is not resident on any processor, the call returns CUDA_SUCCESS and the returned location type for that interval is CU_MEM_LOCATION_TYPE_NONE.

The memory range must refer to one of the following:

  * Managed memory allocated via cuMemAllocManaged, via ::cuMemAllocFromPool from a managed memory pool or declared via **managed** variables.

  * System-allocated pageable memory that is not registered via cuMemHostRegister. If the memory range does not refer to one of the above, the call returns CUDA_ERROR_INVALID_VALUE.

All devices on the system must have non-zero value of device attribute CU_DEVICE_ATTRIBUTE_CONCURRENT_MANAGED_ACCESS. If not, this call returns CUDA_ERROR_NOT_SUPPORTED.
...
Returns in `*data` a boolean that indicates if the memory this pointer is referencing can be used with the GPUDirect RDMA API.
...
Returns in `*data` an integer representing the locality domain ordinal of the allocation, or -1 if the allocation is not localized to a locality domain.
