...
Source: https://docs.nvidia.com/cuda/cuda-driver-api/cuda_driver_api/group__CUDA__COREDUMP.html

#  6.4. Coredump Attributes Control API
...
`` enumerator CU_COREDUMP_DEFAULT_FLAGS ``

`` enumerator CU_COREDUMP_SKIP_NONRELOCATED_ELF_IMAGES ``

`` enumerator CU_COREDUMP_SKIP_GLOBAL_MEMORY ``

`` enumerator CU_COREDUMP_SKIP_SHARED_MEMORY ``

`` enumerator CU_COREDUMP_SKIP_LOCAL_MEMORY ``

`` enumerator CU_COREDUMP_SKIP_ABORT ``

`` enumerator CU_COREDUMP_SKIP_CONSTBANK_MEMORY ``

`` enumerator CU_COREDUMP_GZIP_COMPRESS ``

`` enumerator CU_COREDUMP_FAULTED_CONTEXTS_ONLY ``

`` enumerator CU_COREDUMP_NO_ERRBAR_AT_EXIT ``

`` enumerator CU_COREDUMP_LOG_ONLY ``

`` enumerator CU_COREDUMP_LIGHTWEIGHT_FLAGS ``
...
`` enumerator CU_COREDUMP_ENABLE_ON_EXCEPTION ``

`` enumerator CU_COREDUMP_TRIGGER_HOST ``

`` enumerator CU_COREDUMP_LIGHTWEIGHT ``

`` enumerator CU_COREDUMP_ENABLE_USER_TRIGGER ``

`` enumerator CU_COREDUMP_FILE ``

`` enumerator CU_COREDUMP_PIPE ``

`` enumerator CU_COREDUMP_GENERATION_FLAGS ``

`` enumerator CU_COREDUMP_MAX ``
...
The supported attributes are:
  ...
  * CU_COREDUMP_GENERATION_FLAGS: An integer with values to allow granular control the data contained in a coredump specified as a bitwise OR combination of the following values:
    ...
    * CU_COREDUMP_SKIP_CONSTBANK_MEMORY - Coredump will not include constbank memory.

    * CU_COREDUMP_GZIP_COMPRESS - The generated coredump will be compressed with gzip, and .gz suffix will be appended to the filename, if it’s not a part of it already.

    * CU_COREDUMP_FAULTED_CONTEXTS_ONLY - The coredump will only include contexts that have encountered an exception or a trap.

    * CU_COREDUMP_NO_ERRBAR_AT_EXIT - By default, when coredumps are requested, the GPU will ensure memory faults and other errors prevent warps from exiting, if possible. This can potentially affect the performance of the application. Setting this flag will disable this functionality, making it possible for faulted warps to exit, but also avoiding the potential performance hit.

    * CU_COREDUMP_LOG_ONLY - Setting this flag will disable actual generation of the coredump file, but exception details will still be logged.
...
`` typedef void (*CUcoredumpStatusCallback)(void *userData, int pid, CUdevice dev) ``
...
\- Process ID of the faulted process

Param dev

\- Device ordinal where the coredump occurred
